...
public:
    ...
    void init(ros::NodeHandle &nh){
        ...
        nh.param<double>("/sensor_height", sensor_height_, 1.723);
        nh.param<bool>("/patchwork/verbose", verbose_, false);

        nh.param("/patchwork/num_iter", num_iter_, 3);
        nh.param("/patchwork/num_lpr", num_lpr_, 20);
        nh.param("/patchwork/num_min_pts", num_min_pts_, 10);
        nh.param("/patchwork/th_seeds", th_seeds_, 0.4);
        nh.param("/patchwork/th_dist", th_dist_, 0.3);
        nh.param("/patchwork/max_r", max_range_, 80.0);
        nh.param("/patchwork/min_r", min_range_, 2.7); // It indicates bodysize of the car.
        nh.param("/patchwork/uniform/num_rings", num_rings_, 30);
        nh.param("/patchwork/uniform/num_sectors", num_sectors_, 108);
        nh.param("/patchwork/uprightness_thr", uprightness_thr_, 0.5); // The larger, the more strict
        nh.param("/patchwork/adaptive_seed_selection_margin", adaptive_seed_selection_margin_,
        ...
        nh.param<bool>("/patchwork/using_global_elevation", using_global_thr_, true);
        nh.param("/patchwork/global_elevation_threshold", global_elevation_thr_, 0.0);
        ...
        nh.getParam("/patchwork/czm/num_zones", num_zones_);
        nh.getParam("/patchwork/czm/num_sectors_each_zone", num_sectors_each_zone_);
        nh.getParam("/patchwork/czm/num_rings_each_zone", num_rings_each_zone_);
        nh.getParam("/patchwork/czm/min_ranges_each_zone", min_ranges_);
        nh.getParam("/patchwork/czm/elevation_thresholds", elevation_thr_);
        nh.getParam("/patchwork/czm/flatness_thresholds", flatness_thr_);
...
//        cout_params();
        ...
        nh.param("/patchwork/visualize", visualize_, true);
        ...
        PlaneViz      = nh.advertise<jsk_recognition_msgs::PolygonArray>("/gpf/plane", 100);
        revert_pc_pub = nh.advertise<sensor_msgs::PointCloud2>("/revert_pc", 100);
        reject_pc_pub = nh.advertise<sensor_msgs::PointCloud2>("/reject_pc", 100);
